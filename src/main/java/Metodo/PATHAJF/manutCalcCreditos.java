...
public class manutCalcCreditos {
    public static void main(String[] args) throws SQLException {
        Connection con = new Conexao().conexaoBanco();
        ResultSet divida = con.createStatement().executeQuery("select codigo, '{\n" +
                "    \"idIntegracao\": \"INTEGRACAO1\",\n" +
                "    \"manutCalcCreditos\": {\n" +
                "      \"idCreditoTributario\": 143557,\n" + //143217 imovel - 143557 economico
                "      \"idManutencaoCalculo\": '||idmanutencao||'\n" +
                "    }\n" +
                "  }' as nome from manutcalccreditos where codigo > 4001");
        while (divida.next()){
            String itemValor = null;
            String jsonBody = divida.getString("nome");
            String div = divida.getString("codigo");
            String urlPost = "https://tributos.betha.cloud/service-layer-tributos/api/manutCalcCreditos/";
            ...
            con.createStatement().executeUpdate("update manutcalccreditos set protocolo = '"+itemValor+"' where codigo = "+div);
        }
    }
}
