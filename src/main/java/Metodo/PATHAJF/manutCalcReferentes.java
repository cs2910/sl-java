...
public class manutCalcReferentes {
    public static void main(String[] args) throws SQLException {
        Connection con = new Conexao().conexaoBanco();
        ResultSet divida = con.createStatement().executeQuery("select idlancamento, codigo, '\n" +
                "    {\n" +
                "    \"idIntegracao\": \"INTEGRACAO1\",\n" +
                "    \"manutCalcReferentes\": {\n" +
                "      \"idManutencoesCalculos\": '||idmanutencao ||',\n" +
                "      \"idLancamentos\": '||idlancamento ||',\n" +
                "      \"idEconomicos\": '||idimovel ||',\n" +
                "      \"idPessoas\": '||idpessoas||'\n" +
                "\t  }\n" +
                "  }' as nome\n" +
                "from manutcalcreferentes where codigo > 4001 and idcloud is null order by codigo");
        while (divida.next()){
            String itemValor = null;
            String jsonBody = divida.getString("nome");
            String div = divida.getString("codigo");
            String idlanc = divida.getString("idlancamento");
            String urlPost = "https://tributos.betha.cloud/service-layer-tributos/api/manutCalcReferentes/";
            ...
            con.createStatement().executeUpdate("update manutcalcreferentes set protocolo = '"+itemValor+"' " +
                    "where codigo = "+div+" and idlancamento = "+idlanc);
        }
    }
