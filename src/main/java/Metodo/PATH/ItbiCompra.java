...
public class ItbiCompra {
    public static void main(String[] args) throws SQLException {
        Connection con = new Conexao().conexaoBanco();
        ResultSet divida = con.createStatement().executeQuery("select * from transfImoveisItensCompra where protocolo is null");
        while (divida.next()){
            String idcloud = divida.getString("idcloud");
            String vlVendaAVista = divida.getString("avista");
            String vlFinanciado = divida.getString("financiado");
            String itemValor = null;
            System.out.println(idcloud);
            String urlPost = "https://tributos.betha.cloud/service-layer-tributos/api/transfImoveisItensCompra/";
            ...
            con.createStatement().executeUpdate("update transfImoveisItensCompra set protocolo = '"+itemValor+"' where idcloud = "+idcloud);
        }
    }
}
