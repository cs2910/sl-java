...
public class PessoaFantasia {
    public static void main(String[] args) throws SQLException {
        Connection con = new Conexao().conexaoBanco();
        ResultSet divida = con.createStatement().executeQuery("SELECT idcloud,'{\"idIntegracao\": \"INTEGRACAO1\",\"pessoas\": {\"idGerado\": {\"id\": '||idcloud ||'},\"nomeFantasia\": \"'||nomefantasia||'\"}}' as nome \n" +
                "FROM pessoas where protocolo is null;");
        while (divida.next()){
            ...
            con.createStatement().executeUpdate("update pessoas set protocolo = '"+itemValor+"' where idCloud = "+div);
        }
    }
}
