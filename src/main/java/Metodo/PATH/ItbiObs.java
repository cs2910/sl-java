...
public class ItbiObs {
    public static void main(String[] args) throws SQLException {
        Connection con = new Conexao().conexaoBanco();
        ResultSet divida = con.createStatement().executeQuery("select * from transferenciaimoveis where protocolo is null");
        while (divida.next()){
            String idcloud = divida.getString("idcloud");
            String comentario = divida.getString("comentario");
            String itemValor = null;
            System.out.println(idcloud);
            String urlPost = "https://tributos.betha.cloud/service-layer-tributos/api/transferenciaImoveis/";
            ...
            con.createStatement().executeUpdate("update transferenciaimoveis set protocolo = '"+itemValor+"' where idcloud = "+idcloud);
        }
    }
}
